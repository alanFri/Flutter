...
import com.example.camera_plugin.CameraSettingsManager.CameraSettingsHandler;
import com.example.camera_plugin.exceptions.CameraManagerException;
import com.example.camera_plugin.models.FlashMode;
import com.example.camera_plugin.sensors.BasicLuminosityNotifier;
import com.example.camera_plugin.sensors.LuminosityNotifier;
import com.example.camera_plugin.sensors.SensorOrientationListener;
import com.example.camera_plugin.surface.FlutterSurfaceFactory;
...
public class CameraPlugin implements FlutterPlugin, MethodCallHandler, ActivityAware {

  private static final String TAG = CameraPlugin.class.getName();
  ...
  public static void registerWith(Registrar registrar) {
    ...
    new CameraPlugin().setPluginActivity(registrar.activity());
    new CameraPlugin().onAttachedToEngine(registrar.context(), registrar.messenger(), registrar.textures());
  }
  ...
  private void onAttachedToEngine(Context applicationContext, BinaryMessenger messenger, TextureRegistry textureRegistry) {
    ...
    channel = new MethodChannel(messenger, "camera-plugin");
    sensorOrientationChannel = new EventChannel(messenger, "camera-plugin/orientation");
    permissionsResultChannel = new EventChannel(messenger, "camera-plugin/permissions");
    imageStreamChannel = new EventChannel(messenger, "camera-plugin/images");
    luminosityStreamChannel = new EventChannel(messenger, "camera-plugin/luminosity");
