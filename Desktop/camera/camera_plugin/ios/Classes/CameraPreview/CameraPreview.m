...
- (void)setCameraPresset:(CGSize)currentPreviewSize {
    ...
    if (!CGSizeEqualToSize(CGSizeZero, currentPreviewSize)) {
        ...
        presetSelected = [CameraConstants selectVideoCapturePresset:currentPreviewSize session:_captureSession];
    } else {
        // Compute the best quality supported by the camera device
        presetSelected = [CameraConstants selectVideoCapturePresset:_captureSession];
    }
    ...
    _currentPreviewSize = [CameraConstants getSizeForPresset:presetSelected];
