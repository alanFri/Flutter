...
class _CameraWidgetState extends State<CameraWidget>
  ...
  Future<void> initPlatformState() async {
    ...
    if (Platform.isAndroid) {
      _permissionStreamSub =
          CameraPlugin.listenPermissionResult().listen((res) {
        if (res) {
          initPlatformState();
        }
    ...
    hasPermissions = await CameraPlugin.checkPermissions();
    ...
    if (widget.onOrientationChanged != null) {
      _orientationStreamSub = CameraPlugin.getNativeOrientation()
    ...
    _initPhotoSize();
    camerasAvailableSizes = await CameraPlugin.getSizes();
    ...
    // start camera --
    try {
      started = await CameraPlugin.start();
    } catch (e) {
  ...
  _initImageStream() {
    ...
    if (widget.imagesStreamBuilder != null) {
      widget.imagesStreamBuilder(CameraPlugin.listenCameraImages());
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
      future: CameraPlugin.getPreviewTexture(),
    ...
    if (widget.switchFlashMode != null) {
      widget.switchFlashMode.addListener(() async {
        if (widget.switchFlashMode.value != null && started) {
          await CameraPlugin.setFlashMode(widget.switchFlashMode.value);
        }
      });
    }
  }
  ...
  _initZoom() {
    if (widget.zoom != null) {
      widget.zoom.addListener(() {
        ...
        CameraPlugin.setZoom(widget.zoom.value);
      });
    }
  }
  ...
  _initSensor() {
    widget.sensor.addListener(() async {
      await CameraPlugin.setSensor(widget.sensor.value);
      if (mounted) {
        setState(() {});
      }
    });
  ...
  _initCaptureMode() {
    widget.captureMode.addListener(() async {
      await CameraPlugin.setCaptureMode(widget.captureMode.value);
    });
  }
  ...
  _initAudioMode() {
    ...
    widget.enableAudio.addListener(() async {
      await CameraPlugin.setAudioMode(widget.enableAudio.value);
    });
  }
  ...
  _initAndroidPhotoSize() {
    ...
    selectedAndroidPhotoSize.addListener(() async {
      ...
      await CameraPlugin.setPhotoSize(
  ...
  _initPhotoSize() {
    ...
    widget.photoSize.addListener(() async {
      ...
      await CameraPlugin.setPreviewSize(widget.photoSize.value.width.toInt(),
          widget.photoSize.value.height.toInt());
      var effectivPreviewSize = await CameraPlugin.getEffectivPreviewSize();
  ...
  _initManualBrightness() {
    ...
    _brightnessCorrectionDataSub = brightnessCorrectionData
        ...
        .listen((value) => CameraPlugin.setBrightness(value));
  ...
  _initBrightnessStream() {
    ...
    widget.luminosityLevelStreamBuilder(CameraPlugin.listenLuminosityLevel());
  }
  ...
  _retryStartCamera(int nbTry) async {
    while (!started && !stopping && nbTry > 0) {
      ...
      try {
        started =
            await Future.delayed(Duration(seconds: 2), CameraPlugin.start);
      } catch (e) {
